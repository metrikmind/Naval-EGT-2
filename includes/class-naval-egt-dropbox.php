<?php
/**
 * Classe per l'integrazione con Dropbox API v2 - VERSIONE AGGIORNATA CON FIX COMPLETI + FUNZIONI AGGIUNTIVE
 * Fix per errore HTTP 400 + Debug avanzato + Gestione robusta OAuth + Funzioni complete
 */
...
class Naval_EGT_Dropbox {
    ...
    /**
     * Lista contenuto cartella (per compatibilità)
     */
    public function list_folder_contents($path = '') {
        $result = $this->list_folder($path);
    ...
    /**
     * Crea link condiviso per un file
     */
    public function create_shared_link($path) {
        $data = array(
            'path' => $path,
            'settings' => array(
                'requested_visibility' => array('.tag' => 'public'),
                'audience' => array('.tag' => 'public'),
                'access' => array('.tag' => 'viewer')
            )
        );
        
        return $this->api_call('sharing/create_shared_link_with_settings', $data);
    }
    
    /**
     * Ottiene informazioni sull'utilizzo dello spazio
     */
    public function get_space_usage() {
        return $this->api_call('users/get_space_usage');
    }
    
    /**
     * Ottiene quota spazio rimanente
     */
    public function get_remaining_space() {
        $usage = $this->get_space_usage();
        ...
        $allocation = $usage['data']['allocation'];
        ...
        if (isset($allocation['allocated'])) {
            ...
            return array(
                'success' => false,
                'message' => 'Impossibile determinare lo spazio totale'
            );
        }
    }
    ...
    public function copy($from_path, $to_path) {
        ...
        return $this->api_call('files/copy_v2', $data);
    }
    ...
    public function move($from_path, $to_path) {
        ...
        return $this->api_call('files/move_v2', $data);
    }
    
    /**
     * Lista revisioni file
     */
    public function list_revisions($path, $limit = 10) {
        ...
        return $this->api_call('files/list_revisions', $data);
    }
    
    /**
     * Ripristina revisione file
     */
    public function restore($path, $rev) {
        ...
        return $this->api_call('files/restore', $data);
    }
    ...
    public function path_exists($path) {
        $result = $this->get_metadata($path);
        return $result['success'];
    }
    
    /**
     * Upload file con progress callback (per file grandi)
     */
    public function upload_large_file($local_path, $dropbox_path, $chunk_size = 4194304) {
        if (!$this->is_configured()) {
            return array('success' => false, 'message' => 'Dropbox non configurato');
        }
        
        if (!file_exists($local_path)) {
            return array('success' => false, 'message' => 'File locale non trovato');
        }
        
        $file_size = filesize($local_path);
        
        // Se il file è piccolo, usa upload normale
        if ($file_size <= $chunk_size) {
            return $this->upload_file($local_path, $dropbox_path);
        }
        
        // Upload sessione per file grandi
        $session_id = null;
        $offset = 0;
        
        $file_handle = fopen($local_path, 'rb');
        if (!$file_handle) {
            return array('success' => false, 'message' => 'Impossibile aprire il file');
        }
        ...
        while ($offset < $file_size) {
            ...
            if ($session_id === null) {
                // Avvia sessione upload
                $result = $this->upload_session_start($chunk);
                if (!$result['success']) {
                    fclose($file_handle);
                    return $result;
                }
                ...
                // Continua sessione upload
                $result = $this->upload_session_append($session_id, $offset, $chunk);
                if (!$result['success']) {
                    fclose($file_handle);
                    return $result;
                }
        ...
        // Finalizza upload
        return $this->upload_session_finish($session_id, $offset, $dropbox_path);
    }
    
    /**
     * Avvia sessione upload
     */
    private function upload_session_start($chunk) {
        $headers = array(
            'Authorization: Bearer ' . $this->access_token,
            'Content-Type: application/octet-stream',
            'Dropbox-API-Arg: {}'
        ...
        $ch = curl_init();
        curl_setopt($ch, CURLOPT_URL, self::CONTENT_URL . 'files/upload_session/start');
        ...
        if ($http_code === 200) {
            ...
            return array('success' => false, 'message' => 'Errore avvio sessione upload');
        }
    }
     ...
     * Continua sessione upload
     */
    private function upload_session_append($session_id, $offset, $chunk) {
        $args = array(
        ...
        $ch = curl_init();
        curl_setopt($ch, CURLOPT_URL, self::CONTENT_URL . 'files/upload_session/append_v2');
        curl_setopt($ch, CURLOPT_POST, true);
        curl_setopt($ch, CURLOPT_POSTFIELDS, $chunk);
        curl_setopt($ch, CURLOPT_HTTPHEADER, $headers);
        curl_setopt($ch, CURLOPT_RETURNTRANSFER, true);
        ...
        if ($http_code === 200) {
            ...
            return array('success' => false, 'message' => 'Errore append sessione upload');
        }
    }
     ...
     */
    private function upload_session_finish($session_id, $offset, $dropbox_path) {
        $args = array(
            'cursor' => array(
                'session_id' => $session_id,
                'offset' => $offset
        ...
        $ch = curl_init();
        curl_setopt($ch, CURLOPT_URL, self::CONTENT_URL . 'files/upload_session/finish');
        curl_setopt($ch, CURLOPT_POST, true);
        ...
        if ($http_code === 200) {
            ...
            return array('success' => false, 'message' => 'Errore finalizzazione upload');
        }
    }
    ...
    public function update_access_token($token) {
        ...
        Naval_EGT_Database::update_setting('dropbox_access_token', $token);
        return true;
    }
    
    /**
     * Aggiorna configurazione app
     */
    public function update_app_config($app_key, $app_secret) {
        $this->app_key = $app_key;
        $this->app_secret = $app_secret;
        Naval_EGT_Database::update_setting('dropbox_app_key', $app_key);
        Naval_EGT_Database::update_setting('dropbox_app_secret', $app_secret);
        return true;
    }
    
    /**
     * Pulisce configurazione Dropbox
     */
    public function clear_config() {
        ...
        Naval_EGT_Database::update_setting('dropbox_access_token', '');
        Naval_EGT_Database::update_setting('dropbox_app_key', '');
        Naval_EGT_Database::update_setting('dropbox_app_secret', '');
        
        return true;
    }
    
    /**
     * Valida token di accesso
     */
    public function validate_token() {
        if (!$this->is_configured()) {
            return array('success' => false, 'message' => 'Token non configurato');
        }
